...
/**
  ...
  */
class FlightRepository(var flights: List[Flight]) {

  def getRoundTripFlights(origin: Location, destination: Location, departure: Date, arrival: Date): Option[List[Flight]] =
    return Option.apply(flights.filter(filterRoundTrip(_, origin, destination, departure, arrival)))

  def getOneWayFlights(origin: Location, destination: Location, departure: Date): Option[List[Flight]] =
    return Option.apply(flights.filter(filterOneWay(_, origin, destination, departure)))

  private def filterOneWay(flight: Flight, origin: Location, destination: Location, departure: Date): Boolean =
    return flight.outbound.origin.equals(origin) && flight.outbound.destination.equals(destination) &&
  ...
  def filterRoundTrip(flight: Flight, origin: Location, destination: Location, departure: Date, arrival: Date): Boolean =
    return flight.outbound.origin.equals(origin) && flight.outbound.destination.equals(destination) && compareDay(flight.outbound.departure, departure) &&
